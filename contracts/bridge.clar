...
;;(use-trait STX)
(use-trait sip-010-token 'STXWGJQ101N1C1FYHK64TGTHN4793CHVKRW3ZGVV.sip-010.sip-010-trait)
(use-trait ownable-trait .ownable.ownable-trait)
(use-trait bridge-token .bridge-token.bridge-token-trait)
...
(define-constant TOKEN-TYPE-BASE u100);;A standard token type supported by the bridge.
(define-constant TOKEN-TYPE-NATIVE u200);;A token directly supported by the blockchain
(define-constant TOKEN-TYPE-WRAPPED u300);; A token that is backed by another token on a different chain.

;; Encoded chain id
;;acts as an encoded chain identifier, helping to distinguish different blockchain networks.
(define-constant THIS-CHAIN 0x53544B53) ;;Defines the encoded chain ID for the Stacks blockchain "STKS" in hexadecimal
;; Token system precision
(define-constant SYSTEM-PRECISION u9)

(define-constant UNLOCK 0x756E6C6F636B) ;; "unlock" It ensures a fixed-length representation of strings
(define-constant VERSION 0x01) ;; Version constant -It's used as a version identifier, so the contract can track its own version

(define-constant BP u10000) ;; 10 bp = 0.1 % ;;Used for fee calculations where 10 BP = 0.1%.
;;It checks if the contract is deployed on a specific principal-If true: It sets ENV-VERSION to 0x1a (Testnet).
;; If false: It sets ENV-VERSION to 0x1b (Mainnet)
;;This helps the contract know which network it is running on.
(define-constant ENV-VERSION (if (is-standard 'STB44HYPYAT2BB2QE513NSP81HTMYWBJP02HPGK6) 0x1a 0x1b)) ;; 0x1a - testnet, 0x1b - mainnet

(define-data-var base-fee-rate-bp uint u10);;Fee rate is defined in basis points (0.1% by default).
(define-data-var validator-public-key (buff 33) 0x00);;Stores the validator public key (33-byte buffer)
...
;; BIG RED BUTTON
(define-data-var is-bridge-enabled bool true);;BIG RED BUTTON to enable or disable the bridge.

;;Stores locked tokens before bridging.
;; locks map
(define-map locks
  {lock-id: (buff 16)}
  {
    sender: principal,;;Address locking the token
    recipient: (buff 32),;;Destination address
    amount: uint,;; Amount locked
    destination: (buff 4),;;4-byte identifier for the destination chain
    token-source: (buff 36);; 36-byte token address
  }
)
;; unlocks map
(define-map unlocks
  {chain-lock-id: (buff 20)};;20-byte lock ID from another chain
  {value: bool};;true if already unlocked.
)

;; supported assets map
;; ;Maps supported tokens to their properties
(define-map assets
  {address: principal} ;;Principal of the token contract.

  {
    token-source: (buff 36),;;Token identifier (like a symbol or address)
    precision: uint,
    token-type: uint,;;Base, native, or wrapped.
    min-fee: uint;; transaction fee.
  }
)
;;Maps token sources to contract addresses
;; asset source map
...
;;Allows adding a token to the bridge.
(define-public 
  ...
  (begin
    (let (
      ...
      (try! (assert-valid-token-input token-source token-address precision type min-fee)) ;;Validates token input (checking format, precision, etc.).
      (if (or (is-eq type TOKEN-TYPE-WRAPPED) ;;Checks if the token type is either Wrapped or Base
              (is-eq type TOKEN-TYPE-BASE))
        (asserts! (is-eq (try! (contract-call? token get-contract-owner)) .bridge) (err ERR-WRONG-OWNER)) ;;If so, ensures the contract owner is the bridge.
        (asserts! (is-eq type TOKEN-TYPE-NATIVE) (err ERR-WRONG-TOKEN-TYPE)) ;;If it not Wrapped/Base, ensures it's a Native token
      )
      (ok (try! (save-token token-source token-address precision min-fee type))) ;;Saves the token data to the assets map.
    )
  )
)
...
(define-public
  (lock
    ...
    (amount uint)
    (recipient (buff 32))
    (destination (buff 4)) ;;The identifier of the target blockchain
  )
  (begin
